...
if cmp -s "$MICRO_LOG" "$FEMTO_LOG"; then
  FULL_MATCH="yes"
else
  FULL_MATCH="no"
fi

if diff -u "${OUT_DIR}/microgpt.losses" "${OUT_DIR}/femtogpt.losses" >"${OUT_DIR}/losses.diff"; then
  LOSSES_MATCH="yes"
else
  LOSSES_MATCH="no"
fi

if diff -u "${OUT_DIR}/microgpt.samples" "${OUT_DIR}/femtogpt.samples" >"${OUT_DIR}/samples.diff"; then
  SAMPLES_MATCH="yes"
else
  SAMPLES_MATCH="no"
fi
...
num docs:    micro=${MICRO_DOCS} femto=${FEMTO_DOCS}
vocab size:  micro=${MICRO_VOCAB} femto=${FEMTO_VOCAB}
num params:  micro=${MICRO_PARAMS} femto=${FEMTO_PARAMS}

first loss:  micro=${MICRO_STEP1} femto=${FEMTO_STEP1}
last loss:   micro=${MICRO_LAST} femto=${FEMTO_LAST}
step count:  micro=${MICRO_STEP_COUNT} femto=${FEMTO_STEP_COUNT}
sample count: micro=${MICRO_SAMPLE_COUNT} femto=${FEMTO_SAMPLE_COUNT}

full log match: ${FULL_MATCH}
loss trace match: ${LOSSES_MATCH}
sample output match: ${SAMPLES_MATCH}
